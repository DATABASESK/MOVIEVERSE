...
/*export const getRomanticMovies  = async (page = 1) => {
  ...
  const url = `https://api.themoviedb.org/3/discover/movie?language=en-US&with_genres=10749&include_adult=true&sort_by=popularity.desc&page=${validPage}&region=PH&with_original_language=tl&api_key=38248c047fd8ef9b0a7e25d40651e870`;
  ...
  try {
    ...
    console.error("Error fetching Vivamax-style movies:", error);
    return [];
  }
};*/
...
export const getRomanticMovies = async (page = 1) => {
  ...
  const urls = [
    `https://api.themoviedb.org/3/discover/movie?language=en-US&with_genres=18,10749&include_adult=true&sort_by=popularity.desc&page=${validPage}&region=PH&with_original_language=tl&api_key=38248c047fd8ef9b0a7e25d40651e870`,
    `https://api.themoviedb.org/3/discover/movie?language=en-US&with_genres=18,10749&include_adult=true&sort_by=popularity.desc&page=${validPage}&region=US&api_key=38248c047fd8ef9b0a7e25d40651e870`
  ];
  ...
  try {
    ...
    console.error("Error fetching Vivamax-style erotic movies:", error);
    return [];
  }
};
...
export const getInfoTMDB = async (TMDBID, media_type) => {
  ...
  const url = `${baseUrl}/${media_type}/${TMDBID}?language=en-US&api_key=${process.env.TMDB_API_KEY}`;
  ...
  while (attempts < maxRetries) {
    try {
      ...
      const data = await res.json();
      data.type = media_type; // Ensure type is added
      ...
      attempts++;
      console.error(`Attempt ${attempts} - TMDB Fetch Error: ${error.message}`);
